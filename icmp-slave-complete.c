...
int transfer_icmp(HANDLE icmp_chan, unsigned int target, char *out_buf, unsigned int out_buf_size, char *in_buf, unsigned int *in_buf_size, unsigned int max_in_data_size, unsigned int timeout)
{
	...
	rs = icmp_send(icmp_chan, target, out_buf, out_buf_size, NULL, temp_in_buf, max_in_data_size + ICMP_HEADERS_SIZE, timeout);
	// check received data
	if (rs > 0) {
...
int main(int argc, char **argv)
{
	...
	timeout = 3000;
	//case 'r': [No need to change] send a single test icmp request and then quit
	status = STATUS_OK;
